...
document.querySelector("button.add_new").addEventListener("click", function (e) {
  ...
  //console.log('before', name, price, count)

  if (name && price && count) {
    //console.log('after', name, price, count)
    ...
    //console.log('allafter', name, price, count)
  ...
  if (goods.length) {
    ...
    for (let i = 0; i < goods.length; i++) {
      ...
      <tr class="align-middle">
        ...
        <td>${goods[i][3]}</td>
        <td><button type="button" class="good_deleteb btn btn-danger" data-delete="${goods[i][0]}">&#10006;</button></td>
        <td><button type="button" class="good_delete  btn btn-primary" data-goods="${goods[i][0]}">&#10149;</button></td>
      </tr>
      `
      );
      if (goods[i][4] > 0) {
...
document.querySelector(".list").addEventListener("click", function (e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire("Удалено", "Товар удален", "success");
    }
  });
});
